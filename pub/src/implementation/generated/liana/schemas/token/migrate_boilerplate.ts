...
export const Trivia: t_signatures.Trivia = ($) => ({
    ...
    'comments': _p.deprecated_cc($['comments'], ($) => $.__l_map(($) => ({
        'type': _p.deprecated_cc($['type'], ($) => _p.deprecated_cc($, ($): t_out.Trivia.comments.L.type_ => {
            switch ($[0]) {
                case 'line': return _p.ss($, ($) => ['line', null])
                case 'block': return _p.ss($, ($) => ['block', null])
...
export const Text_Type: t_signatures.Text_Type = ($) => _p.deprecated_cc($, ($): t_out.Text_Type => {
    switch ($[0]) {
        case 'quoted': return _p.ss($, ($) => ['quoted', null])
        case 'apostrophed': return _p.ss($, ($) => ['apostrophed', null])
        case 'undelimited': return _p.ss($, ($) => ['undelimited', null])
        case 'backticked': return _p.ss($, ($) => ['backticked', null])
        default: return _p.au($[0])
    }
})
export const Token_Type: t_signatures.Token_Type = ($) => _p.deprecated_cc($, ($): t_out.Token_Type => {
    switch ($[0]) {
