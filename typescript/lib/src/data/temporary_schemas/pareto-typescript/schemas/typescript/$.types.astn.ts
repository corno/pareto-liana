...
export const $ = sh.modules(
    {
        ...
        "Arguments": sh.module_(sh.t.group({
            "open parenthesis token": sh.prop(sh.t.nothing()),
            "arguments": sh.prop(sh.t.list(sh.t.state({
                "expression": sh.toption(sh.t.component("Expression")),
                "spread": sh.toption(sh.t.group({
                    "expession": sh.prop(sh.t.component("Expression")),
                }))
            }))),
            "close parenthesis token": sh.prop(sh.t.nothing())
        })),

        "Binding Pattern": sh.module_(sh.t.state({
            "identifier": sh.toption(sh.t.component("Identifier")),
            "array binding pattern": sh.toption(sh.t.group({
                "elements": sh.prop(sh.t.list(sh.t.state({
                    "binding element": sh.toption(sh.t.group({
                        
                    })),
                })))
            })),
            "object binding pattern": sh.toption(sh.t.group({
            }))

        })),

        "Block": sh.module_(sh.t.group({
            "statements": sh.prop(sh.t.component("Statements")),
        })),

        "Entity Name": sh.module_(sh.t.state({

        })),

        "Expression": sh.module_(sh.t.group({
        })),

        "JSDoc": sh.module_(sh.t.optional(sh.t.component("Node"))),

        "Modifiers": sh.module_(sh.t.optional(sh.t.list(sh.t.state({
            "readonly": sh.toption(sh.t.nothing()),
            "export": sh.toption(sh.t.nothing()),
            "default": sh.toption(sh.t.nothing()),
        })))),

        "Optional_Type": sh.module_(sh.t.optional(sh.t.component("Type"))),

        "Parameters": sh.module_(sh.t.group({
            "entries": sh.prop(sh.t.list(sh.t.state({
            }))),
        })),
        ...
        "Return Type Annotation": sh.module_(sh.t.group({
        })),

        "Type": sh.module_(sh.t.group({
        })),

        "Type Arguments": sh.module_(sh.t.group({
        })),

        "Type Literal And Interface And Class Body": sh.module_(sh.t.group({
        })),

        "Type Parameters": sh.module_(sh.t.group({
        })),

        "Semicolon": sh.module_(sh.t.group({
        })),

        "Source File": sh.module_(sh.t.group({
            "statements": sh.prop(sh.t.list(sh.t.group({

            }))),
        })),

        "Statement": sh.module_(sh.t.group({
        })),

        "Statements": sh.module_(sh.t.group({
        })),

        "String Literal Or Identifier": sh.module_(sh.t.group({
        })),

        "Variable Declaration List": sh.module_(sh.t.group({
        })),
